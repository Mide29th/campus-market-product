...
export default function Chat() {
  ...
  const conversation = conversations.find(c => c.vendorId === vendorId);
  ...
  const handleSend = () => {
    if (inputText.trim() && vendorId) {
      sendMessage(vendorId, inputText.trim());
      setInputText('');
    }
  };
  ...
  if (!user) return <div className="chat-error">Please login to chat</div>;
